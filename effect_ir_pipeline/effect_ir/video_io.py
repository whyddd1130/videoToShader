...
def _to_tensor_rgb(image: np.ndarray, image_size: int) -> torch.Tensor:
...
def load_video_frames(video_path: Path, frame_indices: list[int], image_size: int) -> torch.Tensor:
    ...
    for frame_index in frame_indices:
        ...
        last_frame = _to_tensor_rgb(frame, image_size)
...
def load_input_image(image_path: Path | None, image_size: int) -> torch.Tensor:
    ...
    image = Image.open(image_path).convert("RGB").resize((image_size, image_size), Image.Resampling.BICUBIC)
