...
ShaderLabb37de06059cb = {}

function ShaderLabb37de06059cb:matchWithId(effectId)
    return 'KFM ShaderLab ShaderLabb37de06059cb' == effectId
end

function ShaderLabb37de06059cb.createWithId(effectId)
    if not ShaderLabb37de06059cb:matchWithId(effectId) then
        return nil
    end
    local o = { drawer = {}, _frame = 0, _progress = 0.0 }
    o = newObject(o, ShaderLabb37de06059cb)
    o:init()
    return o
end

function ShaderLabb37de06059cb:init()
    self.drawer = CGE.TextureDrawer:createWithShader(vs, fs)
end

function ShaderLabb37de06059cb:valueType(index)
    if index == 1 then
...
function ShaderLabb37de06059cb:updateValue(index, val1, val2, val3)
    if index == 1 then
        self._progress = val1
    end
end

function ShaderLabb37de06059cb:resize(width, height)
end

function ShaderLabb37de06059cb:updateTimeAndFrame(time, frame)
    local f = frame or self._frame or 0
    self._frame = f
    self._progress = math.max(0.0, math.min(1.0, f / 124.0))
end

function ShaderLabb37de06059cb:_sendTimeUniforms()
...
function ShaderLabb37de06059cb:render(outFBO, inputTex)
...
function ShaderLabb37de06059cb:onDestroy()
end
