...
function HexagonalBokeh:matchWithId(effectId)
    return 'KFM KSkr HexagonalBokeh' == effectId
end

function HexagonalBokeh.createWithId(effectId)
    if not HexagonalBokeh:matchWithId(effectId) then
        return nil
    end
    local o = {
        drawer = {},
    ...
    o = newObject(o, HexagonalBokeh)
    o:init()
    return o
end

function HexagonalBokeh:init()
    self.blurDrawer = CGE.TextureDrawer:createWithShader(vs, fs)
    self.blurProgram = self.blurDrawer:getProgram()
end

function HexagonalBokeh:updateValue(index, val1, val2, val3)
    if index == 1 then
        -- spotSize
        self.spotSize = val1
    elseif index == 2 then
...
function HexagonalBokeh:resize(width, height)

end

function HexagonalBokeh:customResize(width, height)
...
function HexagonalBokeh:step (edge0, edge1, value)
    return math.min(math.max(0, (value - edge0) / (edge1 - edge0)), 1)
end

function HexagonalBokeh:mix (x, y, a)
    return x + (y - x) * a
end

function HexagonalBokeh:remap(x, a, b)
    return x * (b - a) + a
end

function HexagonalBokeh:render(outFBO, inputTex)
    local viewportSize = {}
    ...
    curFbo:bind()
    self.blurProgram:bind()
    self.blurProgram:sendUniformf('u_Angle', 0.0)
    self.blurProgram:sendUniformf('u_SizeScale', intensity)
    self.blurProgram:sendUniformf('u_Size', self:remap(self.spotSize, 0, 1.2))
    self.blurProgram:sendUniformf('u_Bright', self:remap(self.spotIntensity, 1.0, 5.0))
    self.blurDrawer:drawTexture(inputTex)

    local lastFbo = AESP:takeCachedFrameBuffer(self.currentWidth, self.currentHeight)
    lastFbo:bind()
    self.blurProgram:bind()
    self.blurProgram:sendUniformf('u_Angle', 120.0)
    self.blurProgram:sendUniformf('u_SizeScale', intensity)
    self.blurProgram:sendUniformf('u_Size', self:remap(self.spotSize, 0, 1.2))
    self.blurProgram:sendUniformf('u_Bright', self:remap(self.spotIntensity, 1.0, 5.0))
    ...
    self.blurProgram:sendUniformf('u_Size', self:remap(self.spotSize, 0, 1.2))
    self.blurProgram:sendUniformf('u_Bright', self:remap(self.spotIntensity, 1.0, 5.0))
    self.blurDrawer:drawTexture(lastFbo:texId())

    AESP:recycleCachedFrameBuffer(curFbo)
    AESP:recycleCachedFrameBuffer(lastFbo)
end

function HexagonalBokeh:onDestroy()
end
