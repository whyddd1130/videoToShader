...
ShaderLab7f546e059deb = {}

function ShaderLab7f546e059deb:matchWithId(effectId)
    return 'KFM ShaderLab ShaderLab7f546e059deb' == effectId
end

function ShaderLab7f546e059deb.createWithId(effectId)
    if not ShaderLab7f546e059deb:matchWithId(effectId) then
        return nil
    end
    local o = { drawer = {}, _frame = 0, _progress = 0.0 }
    o = newObject(o, ShaderLab7f546e059deb)
    o:init()
    return o
end

function ShaderLab7f546e059deb:init()
    self.drawer = CGE.TextureDrawer:createWithShader(vs, fs)
end

function ShaderLab7f546e059deb:valueType(index)
    if index == 1 then
...
function ShaderLab7f546e059deb:updateValue(index, val1, val2, val3)
    if index == 1 then
        self._progress = val1
    end
end

function ShaderLab7f546e059deb:resize(width, height)
end

function ShaderLab7f546e059deb:updateTimeAndFrame(time, frame)
    local f = frame or self._frame or 0
    self._frame = f
    self._progress = math.max(0.0, math.min(1.0, f / 124.0))
end

function ShaderLab7f546e059deb:_sendTimeUniforms()
...
function ShaderLab7f546e059deb:render(outFBO, inputTex)
...
function ShaderLab7f546e059deb:onDestroy()
end
