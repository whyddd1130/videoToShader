...
ShaderLabadb02414d2cb = {}

function ShaderLabadb02414d2cb:matchWithId(effectId)
    return 'KFM ShaderLab ShaderLabadb02414d2cb' == effectId
end

function ShaderLabadb02414d2cb.createWithId(effectId)
    if not ShaderLabadb02414d2cb:matchWithId(effectId) then
        return nil
    end
    local o = { drawer = {}, _frame = 0, _progress = 0.0 }
    o = newObject(o, ShaderLabadb02414d2cb)
    o:init()
    return o
end

function ShaderLabadb02414d2cb:init()
    self.drawer = CGE.TextureDrawer:createWithShader(vs, fs)
end

function ShaderLabadb02414d2cb:valueType(index)
    if index == 1 then
...
function ShaderLabadb02414d2cb:updateValue(index, val1, val2, val3)
    if index == 1 then
        self._progress = val1
    end
end

function ShaderLabadb02414d2cb:resize(width, height)
end

function ShaderLabadb02414d2cb:updateTimeAndFrame(time, frame)
    local f = frame or self._frame or 0
    self._frame = f
    self._progress = math.max(0.0, math.min(1.0, f / 124.0))
end

function ShaderLabadb02414d2cb:_sendTimeUniforms()
...
function ShaderLabadb02414d2cb:render(outFBO, inputTex)
...
function ShaderLabadb02414d2cb:onDestroy()
end
