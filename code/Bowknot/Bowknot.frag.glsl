...
// The source renderer's keyframe interpolation is not linear in frame time.
// These points were recovered from the silhouettes of the original Bowknot
// render. The second half mirrors the first around the 180-degree pose.
...
void main()
{
    ...
    vec2 bestSource = vec2(0.0);
    float leftX = -1.0;
    float leftY;
    float leftProjectedX;
    float leftDepth;
    projectedPoint(leftX, screen.y, rotation, leftY, leftProjectedX, leftDepth);
    float leftError = leftProjectedX - screen.x;

    // Bracket every inverse-projection solution and keep the visible surface.
    // This avoids the spurious thin triangles produced by a single Newton solve.
    for (int segment = 0; segment < 32; ++segment) {
        float rightX = -1.0 + 2.0 * float(segment + 1) / 32.0;
        float rightY;
        float rightProjectedX;
        float rightDepth;
        ...
        if (leftError * rightError <= 0.0) {
            ...
            float candidateY;
            float candidateProjectedX;
            float candidateDepth;
            projectedPoint(candidateX, screen.y, rotation,
                           candidateY, candidateProjectedX, candidateDepth);
        ...
        leftX = rightX;
        leftY = rightY;
        leftProjectedX = rightProjectedX;
        leftDepth = rightDepth;
        leftError = rightError;
    }

    vec2 srcUv = bestSource * 0.5 + 0.5;
    vec3 color = texture2D(inputImageTexture, clamp(srcUv, 0.0, 1.0)).rgb;
    gl_FragColor = vec4(color * found, 1.0);
}
